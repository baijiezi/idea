...
public class StocksDailyKLineMA5Dao implements IBaseDao {
    ...
    public List<StocksDailyKLineMA5Entity> getByDate(Date date){
        ...
        try{
            ...
            List<StocksDailyKLineMA5Entity> list = new ArrayList<StocksDailyKLineMA5Entity>();
    ...
    public StocksDailyKLineMA5Entity getByDateAndCode(Date date, String code, Session session){
        try{
            DateFormat format = new SimpleDateFormat("yyyy-MM-dd");
            String dt = format.format(date);
            Query query = session.createQuery(" from StocksDailyKLineMA5Entity s where s.date = '" + dt + "' and s.code = '" + code + "'");
    ...
    public StocksDailyKLineMA5Entity getOneByCode(String code){
        Session session = HibernateUtil.getOpenSession();
        session.beginTransaction();
        try{
            Query query = session.createQuery(" from StocksDailyKLineMA5Entity s where s.code = '" + code + "' order by s.id desc limit 1");
            List<StocksDailyKLineMA5Entity> list = query.list();
            if(list!=null && list.size()>0) {
                return list.get(0);
            }
